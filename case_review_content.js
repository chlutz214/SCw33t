...
var requestVars = ['lockTopBar','caseRevTestMode','caseRevSubject','caseRevBody','caseRevCC','caseRevBCC','caseRevDefaultToAssisted','defaultAssType'];
getVars(JSON.stringify(requestVars), init_caseRev);
...
function init_caseRev() {
	...
	if (lockTopBar) {
		...
		$("div.pbHeader").first().css({'z-index': '1', 'position': 'fixed', 'margin': '-5px 0px 0px 0px', 'width': 'inherit','top':'0px'});
		...
		//var totalPos = 0; var totalEarned = 0; for (var i=0; i < $("select").length; i++) { if ($("select")[i].value !== '' || undefined) { totalEarned+=parseInt($("select")[i].value); totalPos+=5; alert(totalEarned + "/" + totalPos)}}
            ...
            questions.each(
                function() {
					...
					var assType = windowLocation.split(new RegExp("[\&\?]"+"caseType"+"="))[1].split('&')[0];//getURLSub(windowLocation, "caseType");
            if (assType == "Standard" || assType == "Project") {
                $("select:eq(0)").val('Assisted');
            } else if (assType == "Chat") {
							$("select:eq(0)").val('Chat');
						}
        }
